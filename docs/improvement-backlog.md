...
| Signal: `organization_administration:write` alone → org takeover | **PASS** - Tier 0 Critical; rulesets, org runners, base permissions |
...
**Verdict:** contents:write calibration validated. Pin v0.1.0 tagged; check CI uses module fetch (Phase B). Drop `GH_GRAPH_READ_TOKEN` after public flip.
...
| `organization-takeover` single-grant | Done (2026-08-28) - retained Critical; see calibration-notes |
